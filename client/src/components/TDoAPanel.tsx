...
/** Well-known reference frequency presets for quick TDoA targeting */
const FREQUENCY_PRESETS = [
  { label: "WWV 10M", freq: 10000, pb: 1000, desc: "NIST time signal, Fort Collins CO" },
  { label: "WWV 15M", freq: 15000, pb: 1000, desc: "NIST time signal, Fort Collins CO" },
  { label: "WWV 5M", freq: 5000, pb: 1000, desc: "NIST time signal, Fort Collins CO" },
  { label: "WWVH 10M", freq: 10000, pb: 1000, desc: "NIST time signal, Kekaha HI" },
  { label: "CHU 7850", freq: 7850, pb: 1000, desc: "NRC time signal, Ottawa Canada" },
  { label: "DCF77", freq: 77.5, pb: 500, desc: "PTB time signal, Mainflingen Germany" },
  { label: "BBC R4 LW", freq: 198, pb: 10000, desc: "BBC Radio 4, Droitwich UK" },
  { label: "RWM 9996", freq: 9996, pb: 1000, desc: "Russian time signal, Moscow" },
  { label: "BPC 68.5", freq: 68.5, pb: 500, desc: "Chinese time signal, Shangqiu" },
  { label: "JJY 40", freq: 40, pb: 500, desc: "NICT time signal, Fukushima Japan" },
  { label: "MSF 60", freq: 60, pb: 500, desc: "NPL time signal, Anthorn UK" },
  { label: "WWVB 60", freq: 60, pb: 500, desc: "NIST LF time signal, Fort Collins CO" },
] as const;
...
export default function TDoAPanel({
  ...
  useEffect(() => {
    ...
    if (job.status === "complete") {
      ...
      if (job.result?.likely_position) {
        // Build heatmap URL from the job key via our proxy
        const heatmapUrl = job.key
          ? `/api/tdoa/heatmap/${encodeURIComponent(job.key)}/TDoA%20map_for_map.png`
          : undefined;
        // Use contour imgBounds for heatmap projection, or fall back to a default region
        const heatmapBounds = job.contours?.[0]?.imgBounds || undefined;
  ...
  // Auto-select best hosts
  const autoSelectQuery = trpc.tdoa.autoSelectHosts.useQuery(
    { count: 3 },
    { enabled: false }
  );
  ...
  const handleAutoSelect = useCallback(async () => {
    try {
      const result = await autoSelectQuery.refetch();
      if (result.data && result.data.length > 0) {
        // Replace current selection with auto-selected hosts
        onClearHosts();
        result.data.forEach((host: any) => onToggleHost(host));
        toast.success(
          `Auto-selected ${result.data.length} hosts optimized for geographic spread and signal quality`
        );
      } else {
        toast.error("No suitable hosts available for auto-selection");
      }
    } catch {
      toast.error("Failed to auto-select hosts");
    }
  }, [autoSelectQuery, onClearHosts, onToggleHost]);

  // Export TDoA results as JSON
  const handleExportResults = useCallback(() => {
    ...
    const exportData = {
      ...
      jobId: result.jobId,
      position: {
        latitude: result.likelyLat,
        longitude: result.likelyLon,
      },
      ...
      parameters: {
        frequencyKhz: parseFloat(frequencyKhz),
        passbandHz: parseInt(passbandHz),
        sampleTime,
      },
      hostStatuses: hostStatuses.map((hs) => ({
        host: hs.host,
        status: hs.status,
      })),
    };
    ...
    a.href = url;
    a.download = `tdoa-report-${result.jobId.slice(0, 12)}.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    toast.success("TDoA report downloaded");
  }, [result, selectedHosts, frequencyKhz, passbandHz, sampleTime, hostStatuses]);
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      disabled={isRunning || autoSelectQuery.isFetching}
                      className="flex items-center gap-1 text-[10px] text-violet-400 hover:text-violet-300 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                      title="Auto-select 3 best hosts for geographic spread and signal quality"
                    >
                      {autoSelectQuery.isFetching ? (
                ...
                <div className="flex flex-wrap gap-1 mb-2">
                  {FREQUENCY_PRESETS.map((preset) => (
                    <button
                      key={preset.label}
                      onClick={() => {
                        setFrequencyKhz(String(preset.freq));
                        setPassbandHz(String(preset.pb));
                        toast.info(`Preset: ${preset.label} — ${preset.desc}`);
                      }}
                      disabled={isRunning}
