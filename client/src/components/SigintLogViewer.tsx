    ...
    >
      ...
      <div className="px-5 py-4 border-b border-border flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {stationLabel && (
              <p className="text-[10px] font-mono text-muted-foreground/70 mt-0.5 truncate max-w-[300px]">
                {stationLabel}
              </p>
            )}
          </div>
        ...
        <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 bg-foreground/5 rounded-lg p-0.5">
              <button
                ...
                className={`px-2.5 py-1 rounded-md text-[10px] font-mono transition-colors ${
                  viewMode === "chart" ? "bg-foreground/10 text-foreground/90" : "text-muted-foreground/70 hover:text-muted-foreground"
                }`}
              >
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-md text-[10px] font-mono transition-colors ${
                  viewMode === "table" ? "bg-foreground/10 text-foreground/90" : "text-muted-foreground/70 hover:text-muted-foreground"
                }`}
              >
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-md text-[10px] font-mono transition-colors ${
                  viewMode === "all-stations" ? "bg-foreground/10 text-foreground/90" : "text-muted-foreground/70 hover:text-muted-foreground"
                }`}
              >
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-foreground/5 transition-colors text-muted-foreground/70 hover:text-foreground/70">
            <X className="w-5 h-5" />
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {stationLabel && summary && (
          <div className="px-5 py-3 border-border-border5">
          ...
          <div className="px-5 py-4">
            ...
            {bandKeys.length > 0 && (
              <div className="flex items-center gap-1.5 mb-3 flex-wrap">
                <span className="text-[9px] font-mono text-muted-foreground/50 uppercase mr-1">Band:</span>
                <button
            ...
            {entries.some((e) => e.users >= 0) && (
              <div className="mt-4">
                <p className="text-[9px] font-mono text-muted-foreground/50 uppercase mb-2">Active Users Over Time</p>
              ...
              <table className="w-full text-[10px] font-mono">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left py-2 px-2 text-muted-foreground/70 font-medium">Time</th>
                    <th className="text-center py-2 px-2 text-muted-foreground/70 font-medium">Status</th>
                    <th className="text-right py-2 px-2 text-muted-foreground/70 font-medium">SNR</th>
                    <th className="text-right py-2 px-2 text-muted-foreground/70 font-medium">Users</th>
                    <th className="text-right py-2 px-2 text-muted-foreground/70 font-medium">GPS</th>
                    <th className="text-center py-2 px-2 text-muted-foreground/70 font-medium">ADC</th>
                  </tr>
                </thead>
                <tbody>
                  {[...entries].reverse().map((entry, i) => (
                    <tr key={i} className="border-b border-border hover:bg-foreground/[0.03] transition-colors">
                      <td className="py-1.5 px-2 text-muted-foreground">{formatDateTime(entry.ts)}</td>
          ...
          <div className="px-5 py-4">
            {allStations.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-muted-foreground/50">No stations logged yet</p>
              ...
              <div className="space-y-2">
                <p className="text-[10px] font-mono text-muted-foreground/50 uppercase mb-3">
        ...
        {stationLabel && entries.length === 0 && viewMode !== "all-stations" && (
          <div className="text-center py-12 px-5">
            <Activity className="w-8 h-8 text-muted-foreground/30 mx-auto mb-3" />
            <p className="text-sm text-muted-foreground/50">No log entries yet</p>
            <p className="text-[10px] text-muted-foreground/30 mt-1">
      ...
      {/* Footer with actions */}
      <div className="px-5 py-3 border-t border-borderflex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          {stationLabel && entries.length > 0 && (
            <>
              <button
                onClick={handleExportCsv}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-foreground/5 hover:bg-foreground/[0.08] border border-border text-[10px] font-mono text-muted-foreground hover:text-foreground/80 transition-colors"
              >
...
function StatCard({ label, value, icon: Icon, color }: {
  ...
  return (
    <div className="flex items-center gap-2 p-2 rounded-lg bg-foreground/[0.03] border border-border">
      <Icon className="w-3.5 h-3.5 shrink-0" style={{ color: color + "80" }} />
      <div className="min-w-0">
        <p className="text-[8px] font-mono text-muted-foreground/50 uppercase">{label}</p>
        <p className="text-[11px] font-mono font-bold text-foreground/80">{value}</p>
      </div>
    </div>
  );
}
...
function AllStationRow({ station, onRemove }: { station: StationLog; onRemove: () => void }) {
  ...
  return (
    <div className="rounded-lg border border-border bg-foreground/[0.02] overflow-hidden">
      <button
        onClick={() => setExpanded(!expanded)}
        className="w-full px-3 py-2.5 flex items-center gap-3 hover:bg-foreground/[0.03] transition-colors"
      >
        ...
        <div className="flex-1 min-w-0 text-left">
          <p className="text-[11px] text-foreground/80 truncate">{station.stationLabel}</p>
              ...
              {summary && (
                <div className="grid grid-cols-3 gap-2 mb-2">
                  <div className="text-center">
                    <p className="text-[8px] font-mono text-muted-foreground/50 uppercase">Uptime</p>
                    <p className="text-[10px] font-mono text-muted-foreground">{summary.uptimePercent}%</p>
                  </div>
                  <div className="text-center">
                    <p className="text-[8px] font-mono text-muted-foreground/50 uppercase">SNR Range</p>
                    <p className="text-[10px] font-mono text-muted-foreground">{summary.minSnr}–{summary.maxSnr} dB</p>
                  </div>
                  <div className="text-center">
                    <p className="text-[8px] font-mono text-muted-foreground/50 uppercase">Span</p>
                    <p className="text-[10px] font-mono text-muted-foreground">{formatDuration(summary.timeSpanHours)}</p>
                  </div>
                </div>
              )}
  ...
  if (dataPoints.length < 2) {
    return (
      <div className="text-center py-6">
        <p className="text-[10px] text-muted-foreground/30">Not enough data points for chart (need 2+)</p>
      </div>
    );
  }
          ...
          <button
            ...
            className={`text-[8px] font-mono px-1.5 py-0.5 rounded transition-colors border ${
              showBaseline
                ...
                : "text-muted-foreground/50 border-border hover:text-muted-foreground/70"
            }`}
          >
            Baseline
          </button>
